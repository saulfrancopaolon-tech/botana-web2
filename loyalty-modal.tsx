...
export function LoyaltyModal({ isOpen, onClose }: Props) {
  ...
  async function handleRedeem() {
    ...
    try {
      ...
      catch { result = { success: false, message: "Respuesta inválida" } }
      if (result.success) {
        const updated: LocalCard = { ...card, points: card.points + 1, usedCodes: [...card.usedCodes, trimmed] }
        setCard(updated)
        saveCard(updated)
        setCode("")
  ...
  const btnActiveClass = "w-full py-4 rounded-full font-black text-sm tracking-widest uppercase bg-[#E53E3E] text-white hover:bg-[#FF5252] transition-all"
  const btnInactiveClass = "w-full py-4 rounded-full font-black text-sm tracking-widest uppercase bg-white/5 border border-white/10 text-white/25 cursor-not-allowed transition-all"
          ...
          <div className="pt-3">
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                disabled={!igClicked}
                className={igClicked ? btnActiveClass : btnInactiveClass}
              >
          ...
          <div className="pt-2">
            ...
            <div className="grid grid-cols-5 gap-2 mb-2">
              {[...Array(10)].map((_, i) => (
                <div
                  key={i}
                  className={i < card.points
                    ? "aspect-square rounded-full flex items-center justify-center text-xs font-bold bg-[#E53E3E] text-white scale-105 shadow-[0_0_12px_rgba(229,62,62,.4)] transition-all duration-300"
            ...
            <p className="text-center text-xs text-white/25 mb-5 font-mono">
              {card.points}/10
              {card.points >= 10 ? " · 🎉 ¡Tarjeta completa!" : " · faltan " + (10 - card.points) + " puntos"}
            </p>
            {card.points >= 10 ? (
              ...
              <div className="bg-white/[.03] border border-white/[.07] rounded-2xl p-4 mb-4">
                ...
                {codeStatus && (
                  <div className={
                    codeStatus.type === "ok"
                      ? "mt-3 py-2.5 px-3 rounded-xl text-center text-xs font-black uppercase tracking-wider bg-green-500/10 text-green-400 border border-green-500/20"
                      : codeStatus.type === "err"
                      ? "mt-3 py-2.5 px-3 rounded-xl text-center text-xs font-black uppercase tracking-wider bg-red-500/10 text-red-400 border border-red-500/20"
                      : "mt-3 py-2.5 px-3 rounded-xl text-center text-xs font-black uppercase tracking-wider bg-yellow-500/10 text-yellow-400 border border-yellow-500/20"
                  }>
                    {codeStatus.msg}
                  </div>
                )}
